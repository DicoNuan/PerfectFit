...
public class WeekViewCustom extends View {

    private enum Direction {
        NONE, LEFT, RIGHT, VERTICAL
    }

    private int STARTING_HOUR = 5;
    private int ENDING_HOUR = 24;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            switch (mCurrentScrollDirection) {
                ...
                case RIGHT:
                    ...
                    ViewCompat.postInvalidateOnAnimation(WeekViewCustom.this);
                    break;
                case VERTICAL:
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * (ENDING_HOUR-STARTING_HOUR) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * (ENDING_HOUR-STARTING_HOUR) + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
            }
    ...
    public WeekViewCustom(Context context) {
        this(context, null);
    }

    public WeekViewCustom(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WeekViewCustom(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        for (int i = STARTING_HOUR; i < ENDING_HOUR; i++) {
            float top = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * (i - STARTING_HOUR)
                    + mHeaderMarginBottom;
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight = Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / (ENDING_HOUR - STARTING_HOUR)));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * (ENDING_HOUR - STARTING_HOUR) - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight / 2)
            mCurrentOrigin.y = getHeight() - mHourHeight * (ENDING_HOUR - STARTING_HOUR) - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight / 2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = STARTING_HOUR; hourNumber < ENDING_HOUR; hourNumber++) {
                float top = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * (hourNumber - STARTING_HOUR) + mTimeTextHeight / 2 + mHeaderMarginBottom;
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            // Draw the day labels.
            String dayLabelBottom = getDateTimeInterpreter().interpretDate(day);
            ...
            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2,
                    mHeaderTextHeight + mHeaderRowPadding - 10,
                    sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);


            canvas.drawText(dayLabelBottom,
                    startPixel + mWidthPerDay / 2,
                    mHeaderTextHeight + mHeaderRowPadding + 25,
                    sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
    ...
    private Calendar getTimeFromPoint(float x, float y) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0 && x > start && x < startPixel + mWidthPerDay) {
                ...
                day.add(Calendar.HOUR, hour + STARTING_HOUR);
                day.set(Calendar.MINUTE, minute < 0 ? 0 : minute);
                return day;
            }
            startPixel += mWidthPerDay + mColumnGap;
        }
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    // Calculate top.
                    float top = mHourHeight * (ENDING_HOUR - STARTING_HOUR) * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * (ENDING_HOUR - STARTING_HOUR) * bottom / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - mEventMarginVertical;
    ...
    public interface CustomInterpreter extends DateTimeInterpreter {
        public String interpretDay(Calendar date);
    }

    public CustomInterpreter getDateTimeInterpreter() {
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new CustomInterpreter() {
                @Override
        ...
        return (CustomInterpreter) mDateTimeInterpreter;
    }
    ...
    public void goToHour(double hour) {
        ...
        if (hour > (ENDING_HOUR-STARTING_HOUR))
            verticalOffset = mHourHeight * (ENDING_HOUR-STARTING_HOUR);
        else if (hour > 0)
            verticalOffset = (int) (mHourHeight * hour);

        if (verticalOffset > mHourHeight * (ENDING_HOUR-STARTING_HOUR) - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int) (mHourHeight * (ENDING_HOUR-STARTING_HOUR) - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
